...
import { normalizeTimelineTracks } from '../timeline-settings.utils';
import {
	readItem,
	updateItem,
	writeItem,
...
export async function getState() {
	return await readManyItems([...storeNamesMap.keys()]);
}
...
export async function restoreLastState() {
	...
	Array.from(storeNamesMap.entries()).forEach(
		([key, store]: [string, Writable<unknown>], i: number) => {
			if (storageValues !== undefined) {
				const value =
					key === CONSTS.tracksStorageKey
						? normalizeTimelineTracks(storageValues[i])
						: storageValues[i];
				store.set(value);
			}
		}
