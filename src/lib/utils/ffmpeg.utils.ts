...
export async function callFfmpeg(settings: IExportSettings) {
	...
	try {
		...
		const blankVideoReturn = await createBlankVideo(settings);
		...
		const flags = createFfmpegFlags(mediaData, settings, fileName);
		...
		// stop the interval before rendering the complete state so elapsed time cannot keep ticking
		stopTimer();
		...
		stopTimer();
		exportState.set(ExportState.FAILED);
		debugLog(`[FFMPEG] export failed: ${error}`);
	}
}
...
// #region blank
async function createBlankVideo(settings: IExportSettings) {
	...
	const aspectRatio = getAspectRatioInPx(settings);
	flags.push(
		'-f',
		'lavfi',
		'-i',
		`color=size=${aspectRatio}:rate=60:color=black`,
		'-f',
		'lavfi',
		'-i',
...
function createFfmpegFlags(
	...
	const needsVideo = settings.format !== ExportFormat.MP3;
	const needsAudio = settings.format === ExportFormat.MP4 || settings.format === ExportFormat.MP3;
	...
	if (needsVideo) {
		...
		finalVideoStream = outputMap.get(OutputMapKey.OVERLAY)![0];
	}
	...
	if (needsAudio) {
		...
		finalAudioStream = outputMap.get(OutputMapKey.AUDIO_MIX)![0];
	}

	const outputStreams = getOutputStreams(settings, finalVideoStream, finalAudioStream);
	filterComplexString += outputStreams.filterComplex;

	// push final filter_complex into flags array
	flags.push(normalizeFilterComplex(filterComplexString));

	appendOutputFlags(flags, settings, fileName, outputStreams.video, outputStreams.audio);
...
function normalizeFilterComplex(filterComplex: string): string {
	// a trailing semicolon creates an empty filter segment, which ffmpeg reports as "No such filter: ''"
	return filterComplex.replace(/;+$/g, '');
}

function getOutputStreams(
	settings: IExportSettings,
	finalVideoStream: string,
	finalAudioStream: string
): { filterComplex: string; video?: string; audio?: string } {
	switch (settings.format) {
		case ExportFormat.GIF: {
			const outputName = 'outgif';
			const filterComplex = `[${finalVideoStream}]fps=${getGifFps(
				settings.compression
			)},split[gif0][gif1];[gif0]palettegen[p];[gif1][p]paletteuse[${outputName}]`;
			return { filterComplex, video: outputName };
		}
		case ExportFormat.PNG:
			return { filterComplex: '', video: finalVideoStream };
		case ExportFormat.MP3:
			return { filterComplex: '', audio: finalAudioStream };
		case ExportFormat.MP4:
		default:
			return { filterComplex: '', video: finalVideoStream, audio: finalAudioStream };
	}
}

function appendOutputFlags(
	flags: string[],
	settings: IExportSettings,
	fileName: string,
	videoStream?: string,
	...
	switch (settings.format) {
		case ExportFormat.GIF:
			flags.push('-map', `[${videoStream}]`, '-loop', '0', fileName);
			break;
		case ExportFormat.PNG:
			// png export captures the composed timeline frame at the current playhead position
			flags.push(
				'-ss',
				`${msToS(get(currentPlaybackTime)).toFixed(2)}`,
				'-map',
				`[${videoStream}]`,
				'-frames:v',
				'1',
			...
			break;
		case ExportFormat.MP3:
			flags.push('-map', `[${audioStream}]`, '-b:a', getMp3Bitrate(settings.compression), fileName);
			break;
		case ExportFormat.MP4:
		default:
			flags.push(
				...
				`[${audioStream}]`,
				'-crf',
				getCompressionCrf(settings.compression),
				'-preset',
				'veryfast',
				fileName
			);
...
function overlayElements(mediaData: IFfmpegElement[], outputMap: OutputMap): string {
	...
	outputMap.set(OutputMapKey.OVERLAY, [outputName ?? outputMap.get(OutputMapKey.RESET)![0]]);
...
export function downloadOutput() {
	...
	a.href = URL.createObjectURL(
		new Blob([data.buffer as ArrayBuffer], { type: getExportMimeType(settings.format) })
	);
	a.download = getExportFileName(settings);
...
function getAspectRatioInPx(settings: IExportSettings): string {
	const aspectRatio = get(previewAspectRatio);

	// return the mapped aspect ratio to the correct px values
	return getExportResolution(settings.resolution, aspectRatio);
}
...
export async function generateAudioWaveform(file: File, size?: string) {
	...
	const flags = [
		...
		inputName,
		'-filter_complex',
		`aformat=channel_layouts=mono,compand=gain=7:soft-knee=1,showwavespic=s=${imageSize}:colors=#ff981a`,
		'-frames:v',
		'1',
		outputName
	];
