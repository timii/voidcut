...
export async function handleFileUpload(files: FileList) {
	...
	for (const [i, file] of mappedFiles.entries()) {
		...
		const { fileMetadata, filePreviewImage } = await getFileMetadataAndPreviewImage(
			file.type,
			filesArr[i]
		);
		...
		if (file.type === MediaType.Video) {
			fileTimelineFrames = await getVideoTimelineFrames(
				filesArr[i],
				fileMetadata.duration ?? 0,
				filePreviewImage
			);
		...
		availableMedia.update((media) => {
			...
			const index = media.findIndex((el) => el.mediaId === file.mediaId);
			...
			media[index] = {
				...
				timelineFrames: fileTimelineFrames,
				loaded: true,
				...fileMetadata
			};
...
export function handleTimelineMediaDrop(
	media: IMedia,
	dropArea: TimelineDropArea,
	rowIndex?: number,
	startTime?: number
) {
...
// create multiple preview frames for video timeline filmstrips
function getVideoTimelineFrames(
	file: File,
	duration: number,
	...
	return new Promise((resolve) => {
		...
		const video = document.createElement('video');
		const frames: string[] = [];
		const frameCount = getTimelineFrameCount(duration);
		const frameTimes = getTimelineFrameTimes(duration, frameCount);
		...
		canvas.width = CONSTS.timelineFilmstripFrameWidth;
		canvas.height = CONSTS.timelineFilmstripFrameHeight;
		...
		const resolveWithFrames = () => {
			URL.revokeObjectURL(videoSrc);
			resolve(frames.length > 0 ? frames : [fallbackImage]);
		};

		const captureFrameAtTime = (timeInSeconds: number): Promise<void> => {
		...
		video.onerror = () => resolveWithFrames();
	});
}

function getTimelineFrameCount(duration: number): number {
	const durationInSeconds = Math.max(1, Math.ceil(duration / CONSTS.secondsMultiplier));
	const oneFrameEveryTwoSeconds = Math.ceil(durationInSeconds / 2);
	return Math.max(1, Math.min(CONSTS.timelineFilmstripMaxFrames, oneFrameEveryTwoSeconds));
}

function getTimelineFrameTimes(duration: number, frameCount: number): number[] {
	const durationInSeconds = duration / CONSTS.secondsMultiplier;
	...
	return Array.from({ length: frameCount }, (_, index) => {
		...
		return Math.max(0, Math.min(durationInSeconds - 0.05, durationInSeconds * progress));
	});
}
...
function getFileMetadata(
	...
	return new Promise((resolve, reject) => {
		...
		// create data url out of file and pass it as a source to the media element
		convertFileToDataUrl(file)
			.then((fileDataUrl) => {
				placeholderEl.src = fileDataUrl;
			})
...
export function resizeFilePreview(
	file: File,
	width = CONSTS.mediaPoolElementWidth,
	height = CONSTS.mediaPoolElementHeight
): Promise<string> {
	return new Promise((resolve, reject) => {
		if (!FileReader) {
